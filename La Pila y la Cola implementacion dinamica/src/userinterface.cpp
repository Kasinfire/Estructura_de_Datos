...
Queue<char> UserInterface::stringToQueue(const std::string& str) { 
    Queue<char> queue;
...
void UserInterface::printQueue(Queue<char>& queue) {
    while (!queue.isEmpty()){
        cout<< queue.dequeue();
    } 
    cout<<endl; 
...
void UserInterface::insertExpresion() {
    ...
    try{
        ...
        Queue<char> infixQueue = stringToQueue(infixExpression);
...
void UserInterface::printExpresion() {
    ...
    Queue<char> tempQueue = postfixedExpression; 
